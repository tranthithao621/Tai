								    	...
								    	for(int i = start ;i< max;i++){
								    		%>
								    			<tr>
													...
													<td><a href="HuyDangKyServlet?ngayDat=<%= list.get(i).getNgayDat()%>&maVacxin=<%=list.get(i).getMaVacxin() %>" class="btn btn-danger"> Hủy</a></td>
												</tr>
								    		<%
								    	}
	...
	<center>
		<%
			if(total_page > 0){
				%>
					<ul class="pagination">
						<li><a href="LichDatTiemKHServlet?page=1<%= request.getParameter("s") !=null?"&s="+request.getParameter("s"):"" %>">  << </a></li>
							<%
								for(int i = 0;i<total_page;i++){
									%>
										
									    <li class="<%= curent_page == (i + 1)?"active":"" %>" ><a  href="LichDatTiemKHServlet?page=<%=i+1 %><%= request.getParameter("s") !=null?"&s="+request.getParameter("s"):"" %>"><%= i + 1 %></a></li>
									    
									<%
								}
							%>
							<li><a href="LichDatTiemKHServlet?page=<%=total_page %><%= request.getParameter("s") !=null?"&s="+request.getParameter("s"):"" %>">  >> </a></li>
						</ul>
				<%
			}
		%>
