...
<body  background ="images/log.jpg">
	...
	<%
		if(total_page > 0){
			%>
				<ul class="pagination">
			<li><a href="LichSuTiemPhongServlet?page=1<%= request.getParameter("s") !=null ? "&s="+request.getParameter("s"): (request.getParameter("from")!= null && request.getParameter("to") != null)?"&from="+request.getParameter("from")+"&to="+request.getParameter("to"):"" %>">  << </a></li>
				<%
					for(int i = 0;i<total_page;i++){
						%>
							
						    <li class="<%= curent_page == (i + 1)?"active":"" %>" ><a  href="LichSuTiemPhongServlet?page=<%=i+1 %><%= request.getParameter("s") !=null ? "&s="+request.getParameter("s"): (request.getParameter("from")!= null && request.getParameter("to") != null)?"&from="+request.getParameter("from")+"&to="+request.getParameter("to"):"" %>"><%= i + 1 %></a></li>
						    
						<%
					}
				%>
				<li><a href="LichSuTiemPhongServlet?page=<%=total_page %><%= request.getParameter("s") !=null ? "&s="+request.getParameter("s"): (request.getParameter("from")!= null && request.getParameter("to") != null)?"&from="+request.getParameter("from")+"&to="+request.getParameter("to"):"" %>">  >> </a></li>
			</ul>
			<%
		}
	%>
